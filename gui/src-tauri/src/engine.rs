...
impl EngineHandle {
    ...
    /// Tauri port: the old `mpsc::Sender<String>` log channel is replaced by
    /// the global tracing layer (which emits `log_line` Tauri events); the
    /// optional `AppHandle` is kept for future direct emits and to satisfy
    /// the new call sites. All capture/relay logic below is unchanged.
    pub fn start(
        cfg: Config,
        stats: Arc<Stats>,
        ...
        let quic: Option<WindivertHandle> = if quic_need {
            match WindivertHandle::open(worker.quic_filter()) {
                Ok(h) => Some(h),
                Err(e) => {
                    // Via the GuiLogLayer this lands in the LIVE CONSOLE as
    ...
    /// (the GUI calls `Stats::reset()` too; double-reset is harmless).
